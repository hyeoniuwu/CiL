...
protected theorem isSome.bind {α β} {o : Option α} (h : o.isSome) (f : α → Option β) : o.bind f = f (o.get h) := by
...
lemma forall_mem_part {c} {y : Part ℕ} (h1 : y.Dom) (h2 : ∀ x ∈ y, x = c) : c∈y := by
  ...
  contradiction
lemma forall_mem_option {c} {y : Option ℕ} (h1 : y.isSome) (h2 : ∀ x ∈ y, x = c) : c∈y := by
  contrapose h2
  simp
  use y.get h1
  constructor
  ...
  contradiction
theorem Part.eq_some_imp_dom {x} {p : Part ℕ} : p=Part.some x → p.Dom := by
  intro a
  subst a
  exact trivial
...
theorem Option.isSome_iff_mem {β} {o : Option β}: o.isSome ↔ (∃z,z∈o) := by
  ...
  simp [h1]
lemma isSome_iff_not_none {α} {o : Option α} : (¬o=Option.none)↔(o.isSome) := by
  apply Iff.intro
  · intro a
...
@[simp] theorem hnat_to_opt_0' {x} : (Denumerable.ofNat (Option ℕ) (x+1)) = Option.some (x) := by exact rfl
