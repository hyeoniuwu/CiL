...
noncomputable def evaln_clamped (O:ℕ→ℕ) (u:ℕ) : ℕ → Code → ℕ → Option ℕ := λ s c x ↦ do
  let use ← usen O c s x
  if u ≤ use then evaln O s c x else Option.none
...
theorem evaln_clamped_prop_1 (h:(evaln_clamped O u s c x).isSome): (usen O c s x).get (evaln_clamped_prop_1_aux h)≤u := by
  unfold evaln_clamped at h
  simp at h
  have := evaln_clamped_prop_1_aux h
  sorry
