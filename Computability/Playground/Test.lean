...
theorem c_kp54_aux_2 (halts:have dvt := (eval Nat.fzero (dovetail (c_kp54_aux i lb)) Aₚ); dvt.Dom) :
  have dvt := (eval Nat.fzero (dovetail (c_kp54_aux i lb)) Aₚ).get halts
  (evals ((n2l Aₚ) ++ (n2l (dvt.l+1))) i lb).Dom := by
    sorry
...
noncomputable def KP54 : ℕ→ℕ := λ s ↦
  ...
  if s%2=0 then -- then s=2i+2, and we will work on Rᵢ.
    have dvt := eval Nat.fzero (dovetail (c_kp54_aux i lb)) Aₚ
    if halts:dvt.Dom then
      have rf := (dvt.get halts).l -- rf is a natural such that (eval_string ((n2l A) ++ (n2l rf)) i n).Dom.
      have Aₛ := (n2l Aₚ) ++ (n2l (rf+1))
      have A_result := (evals Aₛ i lb).get (c_kp54_aux_2 halts)
    ...
    have dvt := eval Nat.fzero (dovetail (c_kp54_aux i la)) Bₚ
    if halts:dvt.Dom then
      have rf := (dvt.get halts).l
      have Bₛ := (n2l Bₚ) ++ (n2l (rf+1))
      have B_result := (evals (Bₛ) i la).get (c_kp54_aux_2 halts)
