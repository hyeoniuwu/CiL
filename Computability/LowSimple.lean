...
lemma step_preserves_R_mem {j s i X} (h : fs_in X.right j) : fs_in (step s i X).right j := by
  simp [step]; aesop
lemma step_preserves_R_not_mem {j k s X} (h : ¬fs_in X.right j) (hk : k<j) : ¬ fs_in (step s k X).right j := by
  simp [step]
...
¬ fs_in (foldr (step s) X (range k).reverse).right j := by
  ...
  | succ k ih =>
    ...
    simpa using this
lemma split_middle (h : ¬fs_in X.right j) (h2: ∃ x ∈ Wn ∅ j s, x > 2*j) :
...
-/
lemma R_foldr (h : ¬fs_in X.right j) (h2: ∃ x ∈ Wn ∅ j s, x > 2*j) (hs : j+1 ≤ s):
...
-/
theorem mem_A_iff_enumerated {x} : x ∈ A ↔ ∃ i s : ℕ, ( ¬fs_in (C s).right i ∧ i+1 ≤ s ∧
  ...
  · intro h
    ...
    fs_in (foldr (step sM1) (C sM1) ((range' (j + 1) (sM1 - 1 - j)).reverse ++ [j] ++ (range j).reverse)).left x ∧
    fs_in (step sM1 j (foldr (step sM1) (C sM1) (range j).reverse)).left x
   := by
      use sM1-1
      have a2 : sM1-1+1 ≤ sM1 := le_of_eq (Nat.sub_add_cancel sM1G1)

      have a3 := range_3_way_split a2
      have a4 : fs_in (foldr (step sM1) (C sM1) ((range' ((sM1-1) + 1) (sM1 - 1 - (sM1-1))).reverse ++ [(sM1-1)] ++ (range (sM1-1)).reverse)).left x := by
        rw [←a3]
        exact hs
      exact ⟨a2,a4, by
