...
:= by
  ...
  have : (evaln O ((s+1) - h1) c (Nat.pair x.l (h1 + x.r))).isSome  := by
    contrapose h4
    simp at h4
    simp [evaln]
...
0∈(evaln O (s+1) cf  (Nat.pair x.l (rfind'_obtain (en2e h)+x.r)))
