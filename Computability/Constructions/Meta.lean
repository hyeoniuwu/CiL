...
@[simp, evp_simps] theorem c_zero_evp' {O} : evalp O c_zero = fun _ : ℕ => c2n zero := by funext x; simp
...
@[simp, evp_simps] theorem c_succ_evp' {O} : evalp O c_succ = fun _ : ℕ => c2n succ := by funext x; simp
...
@[simp, evp_simps] theorem c_left_evp' {O} : evalp O c_left = fun _ : ℕ => c2n left := by funext x; simp
...
@[simp, evp_simps] theorem c_right_evp' {O} : evalp O c_right = fun _ : ℕ => c2n right := by funext x; simp
...
@[simp, evp_simps] theorem c_oracle_evp {O x} : evalp O c_oracle x = c2n oracle := by simp [c_oracle]
@[simp, evp_simps] theorem c_oracle_evp' {O} : evalp O c_oracle = fun _ : ℕ => c2n oracle := by funext x; simp
...
@[simp, evp_simps] theorem c_pair_evp {O a b} : evalp O c_pair ⟪a, b⟫ = c2n (pair (n2c a) (n2c b)) := by simp [c2n, c_pair, Nat.mul_comm]
@[simp, evp_simps] theorem c_pair_evp' {O} : evalp O c_pair = fun ab : ℕ => c2n (pair (n2c ab.l) (n2c ab.r)) := by simp [c2n, c_pair, Nat.mul_comm]
...
@[simp, evp_simps] theorem c_comp_evp {O a b} : evalp O c_comp ⟪a, b⟫ = c2n (comp (n2c a) (n2c b)) := by simp [c2n, c_comp, Nat.mul_comm]
@[simp, evp_simps] theorem c_comp_evp' {O} : evalp O c_comp = fun ab : ℕ => c2n (comp (n2c ab.l) (n2c ab.r)) := by simp [c2n, c_comp, Nat.mul_comm]
...
@[simp, evp_simps] theorem c_prec_evp {O a b} : evalp O c_prec ⟪a, b⟫ = c2n (prec (n2c a) (n2c b)) := by simp [c2n, c_prec, Nat.mul_comm]; exact rfl
@[simp, evp_simps] theorem c_prec_evp' {O} : evalp O c_prec = fun ab : ℕ => c2n (prec (n2c ab.l) (n2c ab.r)) := by simp [c2n, c_prec, Nat.mul_comm]; exact rfl
...
@[simp, evp_simps] theorem c_rfind'_evp {O c} : evalp O c_rfind' c = c2n (rfind' <| n2c c) := by simp [c2n, c_rfind', Nat.mul_comm]; exact rfl
...
@[simp, evp_simps] theorem c_ev_const_evp {O e x} : evalp O c_ev_const ⟪e, x⟫ = c2n (comp (n2c e) (c_const x)) := by simp [c_ev_const_evp']
...
@[cp] theorem c_comp₃_prim : code_prim c_comp₃ := by unfold c_comp₃; apply_cp
@[simp, evp_simps] theorem c_comp₃_evp {O a b c d} : evalp O c_comp₃ ⟪⟪a,b⟫,⟪c,d⟫⟫ = c2n (comp₃ a b c d) := by
  simp [c_comp₃]; rfl
...
@[simp, evp_simps] theorem c_c_rfind_evp {O} : evalp O c_c_rfind = fun x : ℕ => c2n (c_rfind x) := by simp [c_c_rfind, c_rfind]
end c_c_rfind
...
@[simp, evp_simps] theorem c_dovetailn_evp {O} : evalp O c_dovetailn = fun x ↦ c2n (dovetailn <| n2c x) := by
...
@[simp, evp_simps] theorem c_dovetail_evp {O} : evalp O c_dovetail = fun x ↦ c2n (dovetail <| n2c x) := by
...
@[simp, evp_simps] theorem c_c_ifdom_evp {O} : evalp O c_c_ifdom = fun x ↦ c2n (c_ifdom x.l x.r) := by
