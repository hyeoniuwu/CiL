 ...
 := by
  ...
  have hopt_oracle :
    ...
    (o2n ∘ evaln O (s+1) oracle) := by
      ...
      | inl h => simp [h, Nat.not_lt_of_le h]
      | inr h => simp [h, gt_of_not_le h]
  have hzero_mapped : evalp O zero_mapped cri = (map (o2n ∘ evaln O (s+1) zero) (range (s+1))) := by simp [zero_mapped, hs,hopt_zero]
  have hsucc_mapped : evalp O succ_mapped cri = (map (o2n ∘ evaln O (s+1) succ) (range (s+1))) := by simp [succ_mapped, hs,hopt_succ]
  have hleft_mapped : evalp O left_mapped cri = (map (o2n ∘ evaln O (s+1) left) (range (s+1))) := by simp [left_mapped, hs,hopt_left]
  have hright_mapped : evalp O right_mapped cri = (map (o2n ∘ evaln O (s+1) right) (range (s+1))) := by simp [right_mapped, hs,hopt_right]
  have horacle_mapped : evalp O oracle_mapped cri = (map (o2n ∘ evaln O (s+1) oracle) (range (s+1))) := by simp [oracle_mapped, hs,hopt_oracle]
...
theorem c_evaln_evp_aux_nMod4 {O x n s} :
 ...
 :
  ...
  have bounds_left {elem : ℕ} : Nat.pair (evalp O (ml_1.comp right) ⟪elem, cri⟫) (evalp O (s_1.comp right) ⟪elem, cri⟫) ≤ k := by
    simp [hml,hs]
    exact c_evaln_bounds_left
  have bounds_right {elem : ℕ} : Nat.pair (evalp O (mr_1.comp right) ⟪elem, cri⟫) (evalp O (s_1.comp right) ⟪elem, cri⟫) ≤ k := by
    simp [hmr,hs]
    exact c_evaln_bounds_right
  have bounds_m {elem : ℕ} : Nat.pair (evalp O (m_1.comp right) ⟪elem, cri⟫) (evalp O (s_1.comp right) ⟪elem, cri⟫) ≤ k := by
    simp [hm,hs]
    exact c_evaln_bounds_3
  have bounds_pc_c_sM1 {elem : ℕ} : Nat.pair (evalp O (code.comp right) ⟪elem, cri⟫) (evalp O (sM1.comp right) ⟪elem, cri⟫) ≤ k := by
    simp [hcode,hsM1]
  ...
  have hopt_pair :
   ...
   := by
      ...
      | inr h => simp [h, gt_of_not_le h]
  have hpair_mapped : evalp O pair_mapped cri = (map (opt_pair) (range (s+1))) := by simp [pair_mapped, hs,hopt_pair]
      ...
      | inl h =>
        ...
        | inr hh =>
          ...
          | inr hhh =>
            ...
            simp [hpc_mr_s]
      | inr h => simp [h, gt_of_not_le h]
  have hcomp_mapped : evalp O comp_mapped cri = (map (opt_comp) (range (s+1))) := by simp [comp_mapped, hs,hopt_comp]
  ...
  have hopt_prec :
   ...
   := by
      ...
      | inr h => simp [h, gt_of_not_le h]
  have hprec_mapped : evalp O prec_mapped cri = (map (opt_prec) (range (s+1))) := by simp [prec_mapped, hs,hopt_prec]
   ...
   := by
      ...
      | inl h =>
        ...
        | inr hh =>
          ...
          | inr hhh => simp [hhh]
      | inr h => simp [h, gt_of_not_le h]
  have hrfind'_mapped : evalp O rfind'_mapped cri = (map (opt_rfind') (range (s+1))) := by simp [rfind'_mapped, hs,hopt_rfind']
