...
theorem hnat_2 {o:Option ℕ} (ho: o.isSome) : (o2n o) - 1 = o.get ho := by
  ...
  exact rfl
theorem hnat_3 : n2o 0 = Option.none := by exact rfl
...
@[simp, aesop safe] theorem c_opt_bind_ev_pr(hcf:code_prim cf) (hcg:code_prim cg):code_prim (c_opt_bind cf cg) := by unfold c_opt_bind; repeat (first|assumption|simp|constructor)
...
def c_test :=
  ...
  c_opt_bind succ $
  succ
-- theorem c_test_evp : eval_prim O c_test x = 123 := by
--   simp [c_test]
theorem asd (h:x=y+1) : y=x-1 := by exact Nat.eq_sub_of_add_eq (_root_.id (Eq.symm h))
...
@[simp, aesop safe] theorem c_opt_bind'_ev_pr(hcf:code_prim cf) (hcg:code_prim cg):code_prim (c_opt_bind' cf cg) := by unfold c_opt_bind'; repeat (first|assumption|simp|constructor)
