...
`evalp O (c_list_foldr_param cf) ⟪param, init, lN⟫ =
  (
    List.foldr
...
@[cp] theorem c_list_foldr_param_prim (hcf:code_prim cf) : code_prim (c_list_foldr_param cf) := by
  ...
  apply_cp
lemma auxaux {f: ℕ→ℕ} : List.foldr (fun a b ↦ ⟪a.l, f ⟪a.l, a.r, b.r⟫⟫) ⟪param,init⟫
  (List.zipWith (fun (x y : ℕ) ↦ ⟪x,y⟫) (List.replicate lst.length param) lst) =
  ⟪param,List.foldr (fun a b ↦ f ⟪param,⟪a,b⟫⟫) init lst⟫ := by
    induction lst with
    | nil => simp
    | cons head tail ih =>
      simp
