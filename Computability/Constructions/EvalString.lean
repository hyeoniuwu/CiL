...
def c_evalnc :=
  let bind_prev := left
  let u := left.comp (left.comp bind_prev)
  let s := right.comp (left.comp bind_prev)
  let c := left.comp (right.comp bind_prev)
  let x := right.comp (right.comp bind_prev)

  c_opt_bind
  (c_usen.comp₃ (right.comp right) (left.comp right) (right.comp left)) $
  c_if_le_te.comp₄ right u (c_evaln.comp₃ x c s) (zero)
