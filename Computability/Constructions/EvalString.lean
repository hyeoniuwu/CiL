...
@[simp] theorem c_evalnc_evp {O u s c x} : evalp O c_evalnc ⟪u,s,c,x⟫ = o2n (evalnc O u s c x) := by
  simp [c_evalnc,evalp];
  simp [evalnc]
  congr; funext a_0
  simp only [apply_ite]
...
@[simp] theorem c_c_evals_oracle_evp {O o c x} : evalp O c_c_evals_oracle ⟪o,c,x⟫ =
