...
`@[simp] theorem c_f_evp {O : ℕ → ℕ} : evalp O c_f = f := by ...`
  · theorem which asserts that the code `c_f` implements `f` upon evaluation with `evalp`.

`@[simp] theorem c_f_ev {O : ℕ → ℕ} : eval O c_f = f := by rw [← evalp_eq_eval c_f_prim]; simp only [c_f_evp]`
...
@[simp] theorem c_const_evp {O : ℕ → ℕ} : ∀ n m, evalp O (c_const n) m = n
| 0, _ => rfl
...
@[simp] theorem c_sg_evp {O : ℕ → ℕ} : evalp O c_sg = Nat.sg := by
  simp [c_sg,evalp]
  funext n; induction n with
  | zero => exact rfl
  | succ n _ => simp
@[simp] theorem c_sg_ev {O : ℕ → ℕ} : eval O c_sg = Nat.sg := by rw [← evalp_eq_eval c_sg_prim]; simp only [c_sg_evp]
...
@[simp] theorem c_sg'_evp {O : ℕ → ℕ} : evalp O c_sg' = Nat.sg' := by
  simp [c_sg',evalp]
  funext n; induction n with
  | zero => exact rfl
  | succ n _ => simp
@[simp] theorem c_sg'_ev {O : ℕ → ℕ} : eval O c_sg' = Nat.sg' := by rw [← evalp_eq_eval c_sg'_prim]; simp only [c_sg'_evp]
end Computability.Code
theorem Nat.PrimrecIn.sg {O : ℕ → ℕ} : Nat.PrimrecIn O Nat.sg := by rw [←c_sg_evp]; exact code_prim_prop
theorem Nat.PrimrecIn.sg' {O : ℕ → ℕ} : Nat.PrimrecIn O Nat.sg' := by rw [←c_sg'_evp]; exact code_prim_prop
...
@[simp] theorem c_n2b_evp {O : ℕ → ℕ} : evalp O c_n2b = fun x => if n2b x = true then 1 else 0 := by
  simp [c_n2b]
  unfold Nat.sg; unfold n2b
  aesop
@[simp] theorem c_n2b_ev {O : ℕ → ℕ} : eval O c_n2b = fun x => if n2b x = true then 1 else 0 := by rw [← evalp_eq_eval c_n2b_prim]; simp only [c_n2b_evp]; funext x; aesop
end Computability.Code
end n2b
...
@[simp] theorem c_add_evp {O : ℕ → ℕ} : evalp O c_add = unpaired2 Nat.add := by
  simp [c_add,evalp]
  funext n;
  simp [unpaired2]
  induction n.r with
...
@[simp] theorem c_add_ev {O : ℕ → ℕ} : eval O c_add = unpaired2 Nat.add := by rw [← evalp_eq_eval c_add_prim]; simp only [c_add_evp]
end Computability.Code
end add
section mul
namespace Computability.Code
...
@[simp] theorem c_mul_evp {O : ℕ → ℕ} : evalp O c_mul = unpaired2 Nat.mul := by
  simp [c_mul,evalp]
  funext n;
  simp [unpaired2]
  induction n.r with
...
@[simp] theorem c_mul_ev {O : ℕ → ℕ} : eval O c_mul = unpaired2 Nat.mul := by rw [← evalp_eq_eval c_mul_prim]; simp only [c_mul_evp]
end Computability.Code
end mul
section pow
namespace Computability.Code
...
@[simp] theorem c_pow_evp {O : ℕ → ℕ} : evalp O c_pow = unpaired2 Nat.pow := by
  simp [c_pow,evalp]
  funext n;
  simp [unpaired2]
...
@[simp] theorem c_pow_ev {O : ℕ → ℕ} : eval O c_pow = unpaired2 Nat.pow := by rw [← evalp_eq_eval c_pow_prim]; simp only [c_pow_evp]
end Computability.Code
end pow
...
@[simp] theorem c_pred_evp {O : ℕ → ℕ} : evalp O c_pred = Nat.pred := by
  simp [c_pred]
  funext n;
  cases n <;> simp [*]
@[simp] theorem c_pred_ev {O : ℕ → ℕ} : eval O c_pred = Nat.pred := by rw [← evalp_eq_eval c_pred_prim]; simp only [c_pred_evp]
end Computability.Code
end pred
section sub
namespace Computability.Code
...
@[simp] theorem c_sub_evp {O : ℕ → ℕ} : evalp O c_sub = unpaired2 Nat.sub := by
  simp [c_sub,evalp]
  funext n;
  simp [unpaired2]
  induction n.r with
  ...
  | succ n h =>
    simp [*, Nat.sub_add_eq];
@[simp] theorem c_sub_ev {O : ℕ → ℕ} : eval O c_sub = unpaired2 Nat.sub := by rw [← evalp_eq_eval c_sub_prim]; simp only [c_sub_evp]
end Computability.Code
end sub
section dist
namespace Computability.Code
...
@[simp] theorem c_dist_evp {O : ℕ → ℕ} : evalp O c_dist = unpaired2 Nat.dist := by simp [c_dist,evalp]; exact rfl
...
@[simp] theorem c_if_eq'_evp {O : ℕ → ℕ} : evalp O c_if_eq' = fun ab => if ab.l=ab.r then 0 else 1 := by simp [c_if_eq',evalp];
@[simp] theorem c_if_eq'_ev {O : ℕ → ℕ} : eval O c_if_eq' = fun ab => if ab.l=ab.r then Part.some 0 else Part.some 1 := by
...
theorem c_if_eq_te_evp' {O : ℕ → ℕ} : evalp O c_if_eq_te = fun x => if x.l.l=x.l.r then x.r.l else x.r.r := by
  ...
  | inl h => simp [h]
  | inr h => simp [h]
theorem c_if_eq_te_ev' {O : ℕ → ℕ} : eval O c_if_eq_te = fun x => if x.l.l=x.l.r then x.r.l else x.r.r := by
...
@[simp] theorem c_mul2_evp {O : ℕ → ℕ} : evalp O c_mul2 = fun x => x*2 := by simp [c_mul2]
@[simp] theorem c_mul2_ev {O : ℕ → ℕ} : eval O c_mul2 = (fun x => x*(2:ℕ)) := by rw [← evalp_eq_eval c_mul2_prim]; simp only [c_mul2_evp];
end Computability.Code
end mul2
...
@[simp] theorem c_max_evp {O : ℕ → ℕ} : evalp O c_max = unpaired2 Nat.max := by
  simp [c_max,evalp, -pair_lr]
  exact rfl
@[simp] theorem c_max_ev {O : ℕ → ℕ} : eval O c_max = unpaired2 Nat.max := by rw [← evalp_eq_eval c_max_prim]; simp only [c_max_evp]
end Computability.Code
end max
