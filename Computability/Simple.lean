...
lemma step_preserves_R_mem {j s i X} (h : fs_in X.r j) : fs_in (step s i X).r j := by
  simp [step]; aesop
lemma step_preserves_R_not_mem {j k s X} (h : ¬fs_in X.r j) (hk : k < j) :
    ¬ fs_in (step s k X).r j := by
  ...
  · intro h
    ...
    have hstep : ∃ j, j+1 ≤ sM1 ∧
        fs_in (foldr (step sM1) (C sM1) ((range' (j + 1) (sM1 - 1 - j)).reverse ++
          [j] ++ (range j).reverse)).l x ∧
        fs_in (step sM1 j (foldr (step sM1) (C sM1) (range j).reverse)).l x := by
      use sM1-1
      ...
      have a4 : fs_in (foldr (step sM1) (C sM1)
          ((range' ((sM1-1) + 1) (sM1 - 1 - (sM1-1))).reverse ++
            [sM1-1] ++ (range (sM1-1)).reverse)).l x := by
        rwa [← a3]
