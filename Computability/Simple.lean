...
In this file we specify the construction procedure of a simple set, roughly following Cooper 2004,
proving that it satisfies the positive and negative requirements `P` and `N`.
...
- `Simple.P`: the positive requirement of the simplicity proof, asserting that `A` intersects every
  infinite c.e. set.
...
/-
Alternative characterisation of simplicity of a set; a set is simple iff if it is co-infinite,
computable enumerable, and is such that its complements admits no infinite computably
enumerable subset.
-/
theorem simpleInReq {O a} :
    ((W O a)ᶜ.Infinite ∧ ∀ c, (W O c).Infinite → (W O c ∩ W O a ≠ ∅)) ↔
    simpleIn O (W O a) :=
  ⟨
    fun ⟨h1,h2⟩ =>
      ⟨
        CEin_trivial,
        h1,
        fun c h3 ↦ (nonempt_int_iff_not_subset_compl (W O c) (W O a)).mp (h2 c h3)
      ⟩,
    fun ⟨_, h3, h4⟩ =>
      ⟨h3, fun c h5 ↦ (nonempt_int_iff_not_subset_compl (W O c) (W O a)).mpr (h4 c h5)⟩
  ⟩
...
We split the proof into 4 main sections;
  ...
  · section step_lemmas: basic lemmas about the main loop in the procedure, detailing when
    membership in the constructed sets change
  · section positive_requirements: proves `P`.
  · section negative_requirements: proves `N`, by defining a function `f` which maps elements
    in `A` to the requirement which enumerated them. The bulk of work in this section is dedicated
    to showing that this function is injective.
...
At stage `s` of the construction, we do the following:
  ...
  We also note here that step 2 can be done computably,
  as W_i is finite (elements are bounded by i.)
...
def A : Set ℕ := {x | ∃ s, fs_in (C s).left x}

section step_lemmas
lemma step_preserves_A_mem {j s i X} (h : fs_in X.left j) : fs_in (step s i X).left j := by
  simp [step]; aesop
lemma step_preserves_R_mem {j s i X} (h : fs_in X.right j) : fs_in (step s i X).right j := by
  simp [step]; aesop
lemma step_preserves_R_not_mem {j k s X} (h : ¬fs_in X.right j) (hk : k < j) :
    ¬ fs_in (step s k X).right j := by
  simp only [step, Bool.not_eq_true, Set.mem_setOf_eq, gt_iff_lt]
  ...
  next h_1 =>
    ...
    next h_2 => exact h
  next h_1 => exact h
lemma fold_preserves_R_mem {j s X l} (h : fs_in X.right j) : fs_in (foldr (step s) X l).right j := by
  induction l with
  | nil => simpa
  | cons head tail ih => exact step_preserves_R_mem ih
lemma fold_preserves_R_mem2 {s ywit} (h : fs_in (C s).right ywit) : ∀ i, fs_in (C (s+i)).right ywit := by
  intro i
  induction i with
  | zero => simpa
  | succ i ih =>
    ...
    simpa using this
lemma fold_preserves_R_mem3 {s ywit} (h : fs_in (C s).right ywit) : ∀ s2≥s, fs_in (C (s2)).right ywit := by
  intro s2 hs2
  have a0 : s2 = s+(s2-s) := by grind
  have := fold_preserves_R_mem2 h (s2-s)
  rw [← a0] at this
  exact this
lemma fold_preserves_A_mem {j s X l} (h : fs_in X.left j) : fs_in (foldr (step s) X l).left j := by
  induction l with
  | nil => simpa
  | cons head tail ih => exact step_preserves_A_mem ih

/--
Asserts that if the foldr loop loops over a list whose elements are all `< j`,  then `j` can never
be enumerate into R.
-/
lemma split_lower {j k s X} (h : ¬ fs_in X.right j) (hk : k ≤ j) :
¬ fs_in (foldr (step s) X (range k).reverse).right j := by
  induction k with
  | zero => simp at hk; assumption
  | succ k ih =>
    simp [reversed_range_indt, -foldr_reverse]
    have kk : k ≤ j := Nat.le_of_succ_le hk
    have kk2 : k< j := hk
    have := @step_preserves_R_not_mem _ k s _ (ih kk) kk2
    simpa using this
lemma split_middle {j s X} (h : ¬fs_in X.right j) (h2 : ∃ x ∈ Wn ∅ j s, x > 2 * j) :
    fs_in ((step s) j X).right j := by
...
foldr loop into the "lower" section (i.e. `(range j).reverse`) whereby `x` is not enumerated, the
"middle" section (i.e. `[j]`) where `x` becomes enumerated in a single step, and the upper section
where membership of `x` in `A` is maintained.
-/
lemma range_3_way_split {s j} (hs : j + 1 ≤ s) :
    (range s).reverse = (range' (j+1) (s-1-j)).reverse ++ [j] ++ (range j).reverse := by
...
/--
`R_foldr` states that when conditions are met, `j` will be added to `R` when executing the main
foldr loop.
-/
lemma R_foldr {j s X} (h : ¬fs_in X.right j) (h2 : ∃ x ∈ Wn ∅ j s, x > 2 * j) (hs : j + 1 ≤ s) :
fs_in (foldr (step s) X (range s).reverse).right j := by
  rw [range_3_way_split hs]
  simp? [-foldr_reverse] says
    simp only [append_assoc, cons_append, nil_append, foldr_append, foldr_cons]
...
was added to A.
-/
def RiA (X s : ℕ) := ∀ j, fs_in X.right j → ∃ x ∈ Wn ∅ (n2c j) s, fs_in X.left x
lemma RiA_step (X s : ℕ) : RiA X s → ∀ k, RiA (step s k X) s := by
  simp? [RiA, step] says simp only [RiA, Set.mem_setOf_eq, step, Bool.not_eq_true, gt_iff_lt]
  ...
  next h =>
    ...
    next h_1 =>
      ...
      cases a with
      | inl h_3 =>
        exact (h0 j h_3).elim fun x hx ↦
          ⟨x, @evaln_mono_dom (χ ∅) s s j x (le_refl s) hx.1, Or.inl hx.2⟩
      | inr h_2 =>
...
lemma RiA_foldr {X s} : RiA X s → ∀ j, RiA (foldr (step s) X (range j).reverse) s := by
  ...
  | zero => simpa
  | succ j ih =>
    simpa [reversed_range_indt, -foldr_reverse] using RiA_step _ s ih j
lemma RiA_C : ∀ s, RiA (C s) s := by
  intro s
  induction s with
  | zero => simp [C, RiA]
  | succ s ih =>
    simp only [RiA, C, Set.mem_setOf_eq]
    intro j h
...
the argument goes like this.
  ...
  5A. If it was, we can use the "preserves" theorems above to show that the desired properties
    will hold.
  5B. If not, the conditions are set up exactly for `i` to enumerated into `R`, by above theorems.
-/
lemma P_aux (i : ℕ) : (W ∅ i).Infinite → (∃ s, fs_in (C s).right i ∧ ∃ y ∈ W ∅ i, fs_in (C s).left y) := by
  intro h -- 1.
  -- 2.
  have : ∃ x ∈ W ∅ i, x > 2*i := by
  ...
  use s+i+2+1
  unfold C
  by_cases h1 : fs_in (C (s + i + 2)).right i -- 5.
  · -- 5A
...
theorem P (i : ℕ) : (W ∅ i).Infinite → (W ∅ i ∩ A).Nonempty := by
  ...
  unfold A
  exact Set.inter_nonempty.mpr <|
    hs1.elim fun x hx ↦ ⟨x, hx.1, by simp only [Set.mem_setOf_eq]; use s; exact hx.2⟩
...
lemma A_step_middle {j s x} {X : ℕ}
    ...
    (h4 : ¬fs_in X.right j) :
    fs_in (step s j X).left x := by
  have found : ∃ x, (evalnSet ∅ s (n2c j) x).isSome = true ∧ x > 2 * j :=
    Exists.intro x h2
  unfold step
  simp? [h4, found] says
    simp only [h4, Bool.false_eq_true, not_false_eq_true, ↓reduceIte, Set.mem_setOf_eq, gt_iff_lt,
      found, ↓reduceDIte, pair_l, Nat.testBit_or, Bool.or_eq_true]
  apply Or.inr
  apply fs_in_singleton.mpr
  have a0 : Nat.find found ≤ x := Nat.find_min' found h2
  have a1 : x ≤ Nat.find found := by aesop
...
That is to say, ∃ s, x ∈ (C s).left.

Let `s` be the smallest such stage.

#A. This then, is the exact stage in which `x` was enumerated. (this is lemma `hx`.)

We can dissect further, and determine the exact *step* in the foldr loop in which `x`
was enumerated.

#B. Let i := the exact step. (We define this by noting it must've been enumerated at some step
(lemma hstep), and let i be the smallest such one.)

The minimality of `i` is then used to prove the goal.
-/
theorem mem_A_iff_enumerated {x} :
    x ∈ A ↔ ∃ i s : ℕ, (¬fs_in (C s).right i ∧ i+1 ≤ s ∧
    let cond t := t ∈ Wn ∅ i s ∧ t > 2*i
    cond x ∧ ∀ t<x, ¬ cond t) := by
  constructor
  · intro h
    simp only [A, Set.mem_setOf_eq] at h
    let s := Nat.find h
    ...
    simp only [C] at hs
    -- #A.
    have hx : ¬ fs_in (C sM1).left x := @Nat.find_min _ _ h sM1 (Nat.sub_one_lt_of_lt a0)
    have sM1G1 : sM1 > 0 := by contrapose hs; simp at hs; simp [hs, C]
    -- #B.
    have hstep : ∃ j, j+1 ≤ sM1 ∧
        fs_in (foldr (step sM1) (C sM1) ((range' (j + 1) (sM1 - 1 - j)).reverse ++
          [j] ++ (range j).reverse)).left x ∧
        fs_in (step sM1 j (foldr (step sM1) (C sM1) (range j).reverse)).left x := by
      use sM1-1
      have a2 : sM1-1+1 ≤ sM1 := le_of_eq (Nat.sub_add_cancel sM1G1)
      have a3 := range_3_way_split a2
      have a4 : fs_in (foldr (step sM1) (C sM1)
          ((range' ((sM1-1) + 1) (sM1 - 1 - (sM1-1))).reverse ++
            [sM1-1] ++ (range (sM1-1)).reverse)).left x := by
        rwa [← a3]
      exact ⟨a2, a4, by simpa [] using a4⟩
    ...
    | zero =>
      simp only [hi_val] at *
      have a7 := hi_prop.2.2
      simp? [step]  at a7 says
        simp only [step, range_zero, reverse_nil, foldr_nil, Nat.testBit_zero, decide_eq_true_eq,
          Nat.mod_two_not_eq_one, Set.mem_setOf_eq, mul_zero, gt_iff_lt] at a7
      have r0 : (C sM1).right % 2 = 0 := by
        contrapose a7
        simpa [a7] using eq_false_of_ne_true hx
      simp only [r0, ↓reduceIte] at a7
      constructor
      · simpa
      constructor
      · exact sM1G1
      ...
      next hn =>
        have a10 : x = Nat.find hn := by
          simp only [pair_l, Nat.testBit_or, hx, Bool.false_or] at a7
          exact fs_in_singleton.mp a7
        simp? [a10] says
          simp only [a10, Set.mem_setOf_eq, mul_zero, gt_iff_lt, Nat.lt_find_iff,
            nonpos_iff_eq_zero, not_and, not_lt, forall_eq, implies_true, and_true]
        exact ⟨(Nat.find_spec hn).1, fun t ht ↦ fun a ↦ ht t (le_refl t) a⟩ 
      next hn =>
        contrapose hn
        simp at hn
    ...
    | succ iM1 =>
    rw [←hi_val]
    have h_iM1 := @hi_min iM1 (by simp [hi_val])
    simp? [-foldr_reverse] at h_iM1 says
      simp only [Order.add_one_le_iff, append_assoc, cons_append, nil_append, foldr_append,
        foldr_cons, not_and, Bool.not_eq_true] at h_iM1
    -- h_iM1: x is not enumerated by the (i-1)ᵗʰ step of the foldr loop.
    replace h_iM1 := h_iM1 ?_ ?_; all_goals clear hi_min
    ...
    · have := hi_prop.2.1
      ...
      simpa [a13, -foldr_reverse] using hs
    ...
    let (eq:=hprev) prev := (step sM1 iM1 (foldr (step sM1) (C sM1) (range iM1).reverse))
    simp only [this, range'_zero, reverse_nil, nil_append, cons_append, foldr_cons] at h_i
    rw [← hprev] at h_i h_iM1
    clear this a13 a12
    -- h_iM1_R: `i` is not enumerated into R by the (i-1)ᵗʰ step of the foldr loop.
    have h_iM1_R : ¬ fs_in prev.right i := by
      simp only [step, Bool.not_eq_true, Set.mem_setOf_eq, gt_iff_lt] at h_i
      contrapose h_i
      simpa [h_i] using h_iM1
    constructor
    · contrapose h_iM1_R 
    ...
    · omega
    simp only []
    simp only [step, h_iM1_R, Bool.false_eq_true, not_false_eq_true, ↓reduceIte, Set.mem_setOf_eq,
      gt_iff_lt] at h_i
    split at h_i
    next h_found =>
      ...
      have a17 : x = Nat.find h_found := by
        simp only [pair_l, Nat.testBit_or, Bool.or_eq_true] at h_i
        simp only [h_iM1, Bool.false_eq_true, false_or] at h_i
  ...
  · intro h
    ...
    simp only [append_assoc, cons_append, nil_append, foldr_append, foldr_cons]
    by_cases h : fs_in fold_lower.left x
...
lemma N_aux_2 {ywit s x}
    (hs21 : ywit + 1 ≤ s)
    (hs1 : ¬fs_in (C s).right ywit)
    (hs221 : (fun t ↦ t ∈ Wn ∅ (Code.n2c ywit) s ∧ t > 2 * ywit) x) :
    fs_in (C (s+1)).right ywit := by
  have a2 := @split_lower ywit ywit s _ hs1 (le_refl ywit)
  let (eq:=hprev) prev := (foldr (step s) (C s) (range ywit).reverse)
  simp? [C, range_3_way_split hs21, -foldr_reverse] says
    simp only [C, range_3_way_split hs21, append_assoc, cons_append, nil_append, foldr_append,
      foldr_cons]
  rw [← hprev] at a2 ⊢
  have a3 : fs_in (step s ywit prev).right ywit := by
...
lemma hf_injective_aux {x y} (hx : x ∈ A) (hy : y ∈ A) (hxy : x ≠ y) :
    choose (mem_A_iff_enumerated.mp hx) ≠ choose (mem_A_iff_enumerated.mp hy) := by
  have hxs := choose_spec (mem_A_iff_enumerated.mp hx)
  have hys := choose_spec (mem_A_iff_enumerated.mp hy)
  contrapose hxy
  -- we now want to show: if `x` and `y` were enumerated by the same requirement, they are equal.
  rw [hxy] at hxs
  -- s is the stage by which x is enumerated into A
  let s := choose hxs
  let hs := choose_spec hxs
  rw [show choose hxs=s from rfl] at hs
  -- s2 is the stage by which y is enumerated into A
  let s2 := choose hys
...
noncomputable def f {i} : {x // x ∈ A ∧ x ≤ 2*i} → ℕ :=
  fun x => (mem_A_iff_enumerated.mp x.property.left).choose
...
by
  ...
  have : ↑x/2 ≤  i := by omega
  linarith
theorem hf_SetInj {i} : Set.InjOn (@f i) ({x | ↑x∈A ∧ x ≤ 2*i}) :=
  Function.Injective.injOn (hf_injective i)

/--
`N i` is the `i`th negative requirement, stating that `A` is "sparse" enough, which is used to prove
that it is coinfinite later.

The proof is standard, given we have already shown that `f` is an injection from
`A ∩ {x | x ≤ 2*i}` to `{x | x ≤ i}`.
-/
theorem N (i : ℕ) :  Set.ncard (A ∩ {x | x ≤ 2*i}) ≤ i+1 := by
  have a0 :=
    @Set.ncard_le_ncard_of_injOn _ _ _ {x | x ≤ i} (@f i) ?_ (@hf_SetInj i) (Set.finite_le_nat i)
  · let s : Set {x // x ∈ A ∧ x ≤ 2*i} :=
      (@setOf { x // x ∈ A ∧ x ≤ 2 * i } fun x ↦ ↑x ∈ A ∧ ↑x ≤ 2 * i)
...
lemma N_alternate (i : ℕ) : Set.ncard (Aᶜ ∩ {x | x ≤ 2*i}) ≥ i := by
  ...
  simp? at a0 says simp only [Set.diff_inter_self_eq_diff, tsub_le_iff_right] at a0
  have a2 : (Aᶜ ∩ {x | x ≤ 2 * i}) = ({x | x ≤ 2 * i} \ A) := by aesop
  simp only [a2, ge_iff_le]; clear a2
